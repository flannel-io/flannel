...
type Registry interface {
	...
	watchSubnets(ctx context.Context, since int64) (lease.Event, int64, error)
	watchSubnet(ctx context.Context, since int64, sn ip.IP4Net, sn6 ip.IP6Net) (lease.Event, int64, error)
}
...
func (esr *etcdSubnetRegistry) watchSubnets(ctx context.Context, since int64) (lease.Event, int64, error) {
	key := path.Join(esr.etcdCfg.Prefix, "subnets")
	...
	if rch == nil {
		return lease.Event{}, 0, errNoWatchChannel
	}
	...
	if err != nil {
		return lease.Event{}, 0, err
	}

	return evt, wresp.CompactRevision, err
}

func (esr *etcdSubnetRegistry) watchSubnet(ctx context.Context, since int64, sn ip.IP4Net, sn6 ip.IP6Net) (lease.Event, int64, error) {
	key := path.Join(esr.etcdCfg.Prefix, "subnets", subnet.MakeSubnetKey(sn, sn6))
