...
func (nw *network) reCreateVxlan(ctx context.Context) error {
	...
	for {
		...
		if err != nil || len(ifaceAddrs) == 0 {
			log.Errorf("failed to find IPv4 address for interface %s", extIface.Name)
			time.Sleep(backoff)
			backoff = minDuration(backoff*2, maxBackoff)
			continue
		}
		...
		if err != nil || len(ifaceAddrsV6) == 0 {
			log.Errorf("failed to find IPv6 address for interface %s", extIface.Name)
			time.Sleep(backoff)
			backoff = minDuration(backoff*2, maxBackoff)
			continue
		}
